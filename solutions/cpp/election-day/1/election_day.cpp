...
ElectionResult& determine_result(std::vector<ElectionResult>& final_count){
    ...
    size_t index_most_votes = 0;
    for (size_t i = 0; i < final_count.size(); i++){
        if (final_count[i].votes > current_most_votes){
            current_most_votes = final_count[i].votes;
            index_most_votes = i;
        }
        
    }
    ElectionResult& winner = final_count[index_most_votes];
